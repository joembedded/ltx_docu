...
![In der LTX Microcloud dargestellte Loggerdaten](lora_images/00_ziel_ltx_microcloud.png)

*Zielbild: Mess- und Housekeeping-Werte eines LoRaWAN-Loggers in der LTX Microcloud.*

> [!NOTE]
> Die Screenshots entstanden mit The Things Stack Sandbox V3 und ChirpStack V4.18.0. Bezeichnungen oder Positionen können sich in neueren Oberflächen leicht ändern. Entscheidend sind die in dieser Anleitung genannten Felder und Werte.

> [!CAUTION]
> Alle EUIs, Schlüssel, E-Mail-Adressen, URLs und Gerätenamen in Text und Bildern sind Beispiele. Verwenden Sie ausschließlich die Werte Ihres eigenen Loggers und Ihres eigenen Servers. Root Keys und API-Schlüssel sind Geheimnisse.
...
Für LoRaWAN entscheidet bei `Mem_format` das Bit mit dem Wert `128` über Float16. Ohne dieses Bit wird Float32 übertragen. Details stehen in der [LTX LoRa-Payload-Dokumentation](../lora/lora_payload.md) und in der [LTX-Parameter-Referenz](../ltx_parameter/ltx_parameter_referenz.md).
...
Bei lokaler Eingabe speichert `xWrite` die Änderung dauerhaft. Ausführliche Beispiele enthält der Abschnitt [Typ/FPort am Logger einstellen](../lora/lora_payload.md#typfport-am-logger-einstellen).
...
> [!WARNING]
> `@AT+RFS` löscht Modemkonfiguration, gespeicherten Join-Kontext und Nonces. Verwenden Sie es nur bewusst. Danach müssen die OTAA-Daten erneut gesetzt und die verwendeten DevNonces auch auf dem Server zurückgesetzt werden; andernfalls kann der nächste Join als Replay abgewiesen werden.
...
![Anmeldung mit The Things ID](lora_images/01_ttn_anmeldung.png)

*Anmeldung an der The-Things-ID-Seite. Verwenden Sie Ihr eigenes Konto und ein starkes Passwort.*

![Auswahl des europäischen TTN-Clusters](lora_images/02_ttn_cluster_europa.png)
...
![TTN: neue Anwendung hinzufügen](lora_images/03_ttn_anwendung_hinzufuegen.png)

*Eine Anwendung gruppiert die LTX-Endgeräte und deren Integrationen.*

![TTN: Anwendungs-ID und Beschreibung eingeben](lora_images/04_ttn_anwendung_anlegen.png)
...
![TTN: Gerät manuell mit EU868 und LoRaWAN 1.0.4 anlegen](lora_images/05_ttn_geraetetyp.png)

*Manuelle Registrierung mit dem zum LTX-Modem passenden Frequenzplan und der LoRaWAN-Version 1.0.4.*

![TTN: JoinEUI, DevEUI und AppKey aus dem Logger übernehmen](lora_images/06_ttn_geraeteschluessel.png)
...
![TTN: LTX-Uplink-Decoder als Custom JavaScript Formatter](lora_images/07_ttn_uplink_codec.png)
...
![TTN: Uplink-Decoder mit Beispiel-Payload testen](lora_images/08_ttn_uplink_test.png)
...
![TTN: LTX-Downlink-Encoder einrichten](lora_images/09_ttn_downlink_codec.png)
...
![TTN: JSON-Webhook zur LTX Microcloud](lora_images/11_ttn_webhook.png)
...
![Manuelle LoRaWAN-Übertragung mit dem Kommando i](lora_images/12_ttn_logger_testuebertragung.png)
...
![TTN Live Data mit dekodierten LTX-Kanälen](lora_images/13_ttn_live_data.png)
...
![TTN: verwendete DevNonces zurücksetzen](lora_images/14_ttn_nonces_zuruecksetzen.png)
...
![ChirpStack: Anwendung für LTX-Logger anlegen](lora_images/20_chirpstack_anwendung.png)
...
![ChirpStack: Device EUI, Join EUI und Profil zuordnen](lora_images/23_chirpstack_geraet.png)
...
![ChirpStack: NWKKEY als OTAA Application key eintragen](lora_images/24_chirpstack_appkey.png)
...
![ChirpStack: HTTP-Integration zur LTX Microcloud](lora_images/25_chirpstack_http_integration.png)
...
![Manuelle LoRaWAN-Übertragung mit dem Kommando i](lora_images/26_chirpstack_logger_testuebertragung.png)
...
![Zweiter ChirpStack-Transfer erfolgreich und Debugmodus aus](lora_images/29_chirpstack_zweiter_transfer.png)
...
![ChirpStack-Eventliste mit FCnt, FPort und Rohdaten](lora_images/30_chirpstack_eventliste.png)
...
![ChirpStack zeigt dekodierte LTX-Mess- und HK-Kanäle](lora_images/31_chirpstack_decodierte_daten.png)
...
- [LTX-Parameter-Referenz](../ltx_parameter/ltx_parameter_referenz.md)
